...
def tomo_prep(cfg, verbose_output=False, write_to_disk=True):
    ...
    step = 720
    if verbose_output:
        print(f"use step of {step} to avoid memory overflow")
    ...
    for i_start in range(0, proj.shape[0], step):
        ...
        with cf.ProcessPoolExecutor(max_workers=_cpus) as e:
            _jobs = [
                e.submit(denoise, proj[n, :, :].astype(float))
    ...
    if mode in ["lite", "royal"]:
        ...
        if verbose_output:
            print(f"corrupted frames ind:{idx_bad}")
    ...
    if write_to_disk:
        if verbose_output:
            print(f"writing data back to {h5fn}")
        with h5py.File(h5fn, "a") as _h5f:
            _dst_omegas = _h5f.create_dataset("/tomoproc/omegas", data=omegas)
            _dst_corrm = _h5f.create_dataset(
                "/tomoproc/m_corr_drift", data=m_corr_drift
            )
        ...
        if verbose_output:
            print(f"Building processing graph")
...
def tomo_recon(cfg, verbose_output=False):
    ...
    try:
        if verbose_output:
            print("Try to located pre-processed sinogram...")
        ...
        proj, omegas, index_good, _nodes, _edges = tomo_prep(
            cfg, verbose_output=verbose_output, write_to_disk=False
        )
    # --
    if verbose_output:
        print("Locate rotation center...")
    ...
    # --
    if verbose_output:
        print("write to HDF5 archive")
    with h5py.File(h5fn, "a") as h5f:
        _dst_recon = h5f.create_dataset(
            "/tomoproc/recon_auto",
            data=recon,
            chunks=True,
            compression="gzip",
