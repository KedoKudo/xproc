...
def denoise(
    sino: np.ndarray,
    method: str = "smd",
    config: dict = {"kernel_size": 3, "threshold": 0.1,},
) -> np.ndarray:
    """
    Description
    -----------
...
def remove_corrupted_projs(
    projs: np.ndarray, omegas: np.ndarray, idx_good: Optional[np.ndarray] = None,
...
def correct_horizontal_jittering(
    projs: np.ndarray, omegas: np.ndarray, remove_bad_frames: bool = True,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Description
    -----------
    ...
    # 180 -> 360
    cnts = cnts + cnts
    shift_vals = [np.array([0, projs.shape[2] / 2 - val]) for val in cnts]

    # shift each proj so that the rotation center is the central col
    for n in range(len(shift_vals)):
        projs[n, :, :] = shift(
            projs[n, :, :], shift_vals[n], mode="constant", cval=0, order=1,
        )
...
def correct_detector_drifting(projs: np.ndarray,) -> Tuple[np.ndarray, list]:
    """
    Description
    -----------
    ...
    with cf.ProcessPoolExecutor() as e:
        _jobs = [
            e.submit(detect_slit_corners, projs[n_omega, :, :],)
...
def correct_detector_tilt(
    projs: np.ndarray, omegas: np.ndarray, tor: int = 1, nchunk: int = 4,
) -> np.ndarray:
    """
    Description
    -----------
    ...
    while _err > tor:
        cnt_cols = [
            detect_rotation_center(
                projs[:, n * _st : min((n + 1) * _st, projs.shape[2]), :], omegas
            )
