        ...
        for i in prefs["series"]:
            ...
            filename = "%s/%s%s.%s" % (
                ...
                Transform.paddingZero(
                    int(prefs[kind]["Num"][i][iproj]), prefs["numberDigit"]
                ),
            ...
            dataArray.append(0)
            dataArray[i] = imarray[
                (prefs["ROIy"][i][0] - 1) : prefs["ROIy"][i][-1],
                (prefs["ROIx"][i][0] - 1) : prefs["ROIx"][i][-1],
            ]  # corp the array
            ...
            if prefs["vis"]["intMonitor"]:
                monitor[iproj][i] = array(
                    "d", [aveint, prefs[kind]["Num"][i][iproj], nOut]
                )
...
def projTransform(prefs, nOutstart, sType, kind, fileID, moncnt):
    ...
    sharedArray = Array(
        ctypes.c_double,
        len(prefs[kind]["Num"][0]) * dataResutArray.shape[0] * dataResutArray.shape[1],
        lock=False,
    )
    sharedNpArray = np.frombuffer(sharedArray, dtype=ctypes.c_double)
    finalData = sharedNpArray.reshape(
        len(prefs[kind]["Num"][0]), dataResutArray.shape[0], dataResutArray.shape[1]
    )
    ...
    for i in range(0, procs):
        process = Process(
            target=child,
            args=(i, step, totalimage, prefs, kind, finalData, monitor, nOutstart),
        )
