...
def child(i, procs, step, totalimage, prefs, kind, j, dataAve, monitor, nOut):
    ...
    for iavg in range(Start, End):
        filename = "%s/%s%s.%s" % (
            prefs["filePath"],
            prefs[kind]["Prefix"][0],
            Transform.paddingZero(
                int(prefs[kind]["Num"][j][iavg]), prefs["numberDigit"]
            ),
            prefs[kind]["extension"],
        )
        im = Image.open(filename)
        imarray = np.array(im)
        dataAve[iavg] = imarray[
            (prefs["ROIy"][j][0] - 1) : prefs["ROIy"][j][-1],
            (prefs["ROIx"][j][0] - 1) : prefs["ROIx"][j][-1],
        ]  # corp the array
...
def avgTransform(prefs, nOut, sType, kind, fileID, moncnt):
    ...
    for j in prefs["series"]:
        ...
        sharedArray = Array(
            ctypes.c_double,
            len(prefs[kind]["Num"][j]) * len(prefs["ROIy"][j]) * len(prefs["ROIx"][j]),
            lock=False,
        )
        sharedNpArray = np.frombuffer(sharedArray, dtype=ctypes.c_double)
        dataAve = sharedNpArray.reshape(
            len(prefs[kind]["Num"][j]), len(prefs["ROIy"][j]), len(prefs["ROIx"][j])
        )
        ...
        sharedMonitor = Array(
            ctypes.c_double, totalimage * len(prefs["series"]) * 3, lock=False
        )
        sharedNpMonitor = np.frombuffer(sharedMonitor, dtype=ctypes.c_double)
        ...
        subROIx.append(0)
        subROIy.append(0)
        dataArray[j], subROIx[j], subROIy[j], _ = Transform.preprocess(
            dataArray[j], prefs, j
        )

    dataResultArray, minx, maxx = Transform.stitching(dataArray, prefs)
    # print dataArray
    prefs = Transform.stitching_calcafter(
        prefs, minx, maxx, dataResultArray.shape[1], dataResultArray.shape[0]
    )
