...
def initPyPref(prefs):
    ...
    prefs["NumWF1"] = 10
    # Number of frames for the initial beam profile (white field)
    prefs["NumProj"] = 901
    # Number of frames for the projections
    prefs["NumWF2"] = 10
    # Number of frames for the initial beam profile (white field)
    prefs["NumDarks"] = 10
    # Number of frames for the dark field

    prefs["frameNum"] = (
        prefs["NumWF1"] + prefs["NumProj"] + prefs["NumWF2"] + prefs["NumDarks"]
    )
    # Image numbers in one part of the stitching
    prefs["VolShift"] = 0
    # Shift of the image numbers to the next volume (default 0)
    # %prefs.VolShift = prefs.sN *prefs.frameNum; % One volume shift in image numbers

    prefs["RotationAngle"] = 0.0
    # default 0, Rotation axis tilt correction in degrees, positive: rotating the image CCW, i.e. rotating the axis CW
    prefs["rotAxis"] = [1024]
    # default
    ...
    prefs["rotax"][
        "fittedshift"
    ] = 0  # The shift determined from the FindRotax script. This will be used for the reconstruction as a starter value
    ...
    prefs["projection"] = {}
    prefs["projection"]["Prefix"] = ["J5_In718_Load11_"]
    prefs["firstfile"] = 11063
    prefs["loadstep"] = ""
    prefs["rotax"]["fittedshift"] = 10
    ...
    prefs["out"]["dirname"] = "%s_rec/%s%s/raw" % (
        prefs["userID"],
        prefs["projection"]["Prefix"][0],
        prefs["loadstep"],
    )
    prefs["out"]["filePath"] = (
        "/home/beams12/S1IDUSER/mnt/orthros/%s" % prefs["out"]["dirname"]
    )

    prefs["ROIx"] = [
        array("i", range(180, 1901))
    ]  # different between matlab and python change from d to i
    prefs["ROIy"] = [
        array("i", range(700, 1751))
    ]  # change d to i for easy manipulation
    ...
    prefs["slits"]["tlx"] = 167
    prefs["slits"]["tly"] = 777
    prefs["slits"]["brx"] = 1917
    prefs["slits"]["bry"] = 1681
    prefs["slits"]["ROISizex"] = 150
    prefs["slits"]["ROISizey"] = 150
    ...
    prefs["slits"]["ROIx"] = [
        array(
            ...
            range(
                int(
                    prefs["slits"]["tlx"] - math.floor(prefs["slits"]["ROISizex"] / 2.0)
                ),
                int(
                    prefs["slits"]["tlx"] + math.ceil(prefs["slits"]["ROISizex"] / 2.0)
                ),
            ),
        ),
        array(
            ...
            range(
                int(
                    prefs["slits"]["brx"] - math.floor(prefs["slits"]["ROISizex"] / 2.0)
                ),
                int(
                    prefs["slits"]["brx"] + math.ceil(prefs["slits"]["ROISizex"] / 2.0)
                ),
            ),
        ),
        array(
    ...
    prefs["slits"]["ROIy"] = [
        array(
            ...
            range(
                int(
                    prefs["slits"]["tly"] - math.floor(prefs["slits"]["ROISizey"] / 2.0)
                ),
                int(
                    prefs["slits"]["tly"] + math.ceil(prefs["slits"]["ROISizey"] / 2.0)
                ),
            ),
        ),
        array(
        ...
        array(
            ...
            range(
                int(
                    prefs["slits"]["bry"] - math.floor(prefs["slits"]["ROISizey"] / 2.0)
                ),
                int(
                    prefs["slits"]["bry"] + math.ceil(prefs["slits"]["ROISizey"] / 2.0)
                ),
            ),
        ),
        array(
    ...
    prefs["slits"]["tempFrameSizex"] = 20
    prefs["slits"]["tempFrameSizey"] = 20
    ...
    prefs["slitposcheck"]["projection"]["Num"].append(
        prefs["projection"]["Num"][0][
            0 :: int(math.ceil((len(prefs["projection"]["Num"][0]) - 1) / 72.0))
        ]
    )
    ...
    prefs["correl"]["last"] = (
        prefs["correl"]["first"] + len(prefs["projection"]["Num"]) - 1
    )
    ...
    ts = time.time()
    prefs["out"]["timestamp"] = datetime.datetime.fromtimestamp(ts).strftime(
        "%Y-%m-%d_%H-%M-%S"
    )
    ...
    prefs["slits"]["slitfname"] = "%s/%slastslitsfield.mat" % (
        prefs["out"]["filePath"],
        prefs["projection"]["Prefix"],
    )

    if False:
        prefs["ROIx"] = [
            array("d", range(1, 2049))
        ]  # different between matlab and python
        ...
        print(
            "Number of layers for the quick reconstruction: %d", len(prefs["cropROIy"])
        )
...
def GenImageNums(prefs, start, FOVstep, imgstep):
    ...
    for i in series:
        prefs["beamProfileInitial"]["Num"].append(
            array(
                "d", [x + prefs["VolShift"] for x in range(s0, (s0 + prefs["NumWF1"]))]
            )
        )
        ...
        prefs["projection"]["Num"].append(
            array("d", [x + prefs["VolShift"] for x in imglist])
        )
        s0 = s0 + prefs["NumProj"]
        prefs["beamProfileFinal"]["Num"].append(
            array(
                "d", [x + prefs["VolShift"] for x in range(s0, (s0 + prefs["NumWF2"]))]
            )
        )
        s0 = s0 + prefs["NumWF2"]
        prefs["dark"]["Num"].append(
            array(
                "d",
                [x + prefs["VolShift"] for x in range(s0, (s0 + prefs["NumDarks"]))],
            )
        )
        ...
        if imgstep <= 1:
            ...
            prefs["beamProfileInitial"]["Num"][-1] = prefs["beamProfileFinal"]["Num"][
                -1
            ]
