...
def stitching_calcbefore(prefs):
    ...
    if isinstance(prefs["series"], array):
        ...
        while count < prefs["series"].__len__():
            ...
            prefs["out"]["xposInBBox"].append(
                int(math.ceil((prefs["bboxx"] - orixsize) / 2.0))
                + prefs["translx"][j]
                + prefs["cropROIx"][j][0]
            )
            prefs["out"]["yposInBBox"].append(
                int(math.ceil((prefs["bboxy"] - oriysize) / 2.0))
                + prefs["transly"][j]
                + prefs["cropROIy"][j][0]
            )
...
def preprocess(
    dataArray, prefs, i
):  # outROIx will strictly equals to the index in matlab, if used as index need to minus 1
    outROIx = range(1, len(dataArray.T) + 1)
    outROIy = range(1, len(dataArray) + 1)

    if prefs["shouldRotate"]:
        dataArray = scipy.ndimage.interpolation.rotate(
            dataArray, prefs["RotationAngle"][i]
        )
        outROIx = range(
            1, len(dataArray.T) + 1
        )  # it is still use the same number as matlab, but if using as the index, will need to change
        outROIy = range(1, len(dataArray) + 1)
    ...
    if prefs["shouldCrop"]:
        ...
        outROIx = range(
            (prefs["ROIx"][i][0] + prefs["cropROIx"][i][0] - 1),
            prefs["ROIx"][i][0] + prefs["cropROIx"][i][-1],
        )
        outROIy = range(
            (prefs["ROIy"][i][0] + prefs["cropROIy"][i][0] - 1),
            prefs["ROIy"][i][0] + prefs["cropROIy"][i][-1],
        )
...
def stitching(dataArray, prefs):
    stitchedArray = np.zeros(
        shape=(int(prefs["bboxy"]), int(prefs["bboxx"]))
    )  # prefs['out']['fileType']
    maxx = 0
    maxy = 0
    minx = sys.maxint
    ...
    if isinstance(prefs["series"], array):
        ...
        while count < prefs["series"].__len__():
            j = (
                int(prefs["series"][count]) - 1
            )  # euqal to prefs.series doing this, because returned might be from matlab, which could be a float
            px = int(prefs["out"]["xposInBBox"][j]) + 1
            py = int(prefs["out"]["yposInBBox"][j]) + 1
            dataArray[j] = dataArray[j].astype(int)
            stitchedArray[
                (py - 1) : (py + dataArray[j].shape[0] - 1),
                (px - 1) : (px + dataArray[j].shape[1] - 1),
            ] = dataArray[j]
            minx = min(minx, px)
    ...
    if prefs["shouldRotateAfterStitch"] and (not prefs["doVis"]["stitching"]):
        stitchedArray = scipy.ndimage.interpolation.rotate(
            stitchedArray, prefs["RotationAngleAfterStitch"]
        )
    ...
    if not prefs["doVis"]["stitching"]:
        if isinstance(prefs["series"], array):
            ...
            while count < prefs["series"].__len__():
                ...
                prefs["out"]["rotAxis"].append(
                    prefs["out"]["xposInBBox"][j]
                    - prefs["ROIx"][j][0]
                    + 1
                    - prefs["cropROIx"][j][0]
                    + 1
                ...
                prefs["out"]["shift"].append(
                    (maxx - minx + 1) / 2.0 - prefs["out"]["rotAxis"][j]
                )
                count = count + 1
    else:
        if isinstance(prefs["series"], array):
