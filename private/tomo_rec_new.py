...
if __name__ == '__main__':
    ...
    if rec_type != 'full':
        ...
        for rotcenter in np.linspace(rot_center - rc_step * 10, rot_center + rc_step * 10, 21):
            ...
            print('Center of Rot[%2d]: %.2f, %.1f, %.1f, %.1f,' % (i, rotcenter, focus_score[1,i], focus_score[2,i], focus_score[3,i]))
        ...
        ax[0].plot(rotcen, fscore[:,0],'o-', color=colors[0], label='%d (+0)' % (sino[0]))
        ax[0].grid(True, ls='--')
        ax[0].legend(loc=0)
        ax[0].set_title('image rotation is %2.3f deg' % rot_angle)
        ax[1].plot(rotcen, fscore[:,1],'o-', color=colors[1], label='%d (+%d)' % (sino[0]+sino[2], sino[2]))
        ax[1].grid(True, ls='--')
        ax[1].legend(loc=0)
        ax[2].plot(rotcen, fscore[:,2],'o-', color=colors[2], label='%d (+%d)' % (sino[0]+sino[2]*2, sino[2]*2))
        ax[2].grid(True, ls='--')
        ax[2].legend(loc=0)
        ...
        if show_rec_img:
            ...
            rectop = cv2.normalize(rectop, None, alpha=-clim[0]*c_scaling16, beta=(65535-clim[0])*c_scaling16, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_16U)
            recmid = cv2.normalize(recmid, None, alpha=-clim[0]*c_scaling16, beta=(65535-clim[0])*c_scaling16, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_16U)
            recbot = cv2.normalize(recbot, None, alpha=-clim[0]*c_scaling16, beta=(65535-clim[0])*c_scaling16, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_16U)
