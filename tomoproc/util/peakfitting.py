...
def gauss1d(x: np.ndarray, amp: float, mu: float, sigma: float) -> np.ndarray:
    ...
    """
    return (
        amp / sigma / np.sqrt(2 * np.pi) * np.exp(-((x - mu) ** 2) / 2 / (sigma ** 2))
    )
...
def voigt1d_convolve(
    x: np.ndarray, amp: float, mu: float, sigma: float, gamma: float,
) -> np.ndarray:
    """
    Description
    -----------
    ...
    """
    return np.convolve(
        gauss1d(x, 1, mu, sigma), lorentz1d(mu - x, amp * gamma, mu, gamma), "same",
    )


def voigt1d(
    x: np.ndarray, amp: float, pos: float, fwhm: float, shape: float,
) -> np.ndarray:
    """
    Description
    -----------
...
def fit_peak_1d(
    xdata: np.ndarray, ydata: np.ndarray, engine: str = "lmfit",
) -> np.ndarray:
    """
    Description
    -----------
...
def sigmoid(x: np.ndarray, xc: float = 0, a: float = 1,) -> np.ndarray:
    """
    Description
    -----------
...
def fit_sigmoid(xdata: np.ndarray, ydata: np.ndarray,) -> Tuple[np.ndarray, np.ndarray]:
    """
    Description
    -----------
    ...
    return curve_fit(
        ...
        ydata,
        p0=[r, 1],
        bounds=([r - r * 0.2, -np.inf], [r + r * 0.2, np.inf],),
    )
...
if __name__ == "__main__":
    ...
    ydata = (
        voigt1d(xdata, amp, pos, fwhm, shape) + (np.random.random(N) - 0.5) * amp / 10
    )
